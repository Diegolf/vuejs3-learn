    ...
    <div class="items">
      <template v-if="productsInBag.length > 0">
        <div
          ...
          :key="index"
        >
          <div class="remove" @click="removeProductFromBag(product.id)">
            Remover Produto
          </div>
        ...
        <div class="grand-total">
          Total do pedido: R$ {{ this.bagTotalPrice.toFixed(2) }}
        </div>
      </template>
      <h4 v-else>Nenhum item adicionado ao carrinho</h4>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    removeProductFromBag(productId) {
      this.$store.dispatch("removeProductFromBag", productId);
    },
  },
};
</script>
